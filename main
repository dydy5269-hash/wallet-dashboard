...
export default function WalletDashboard() {
  ...
  async function saveToGist() {
    ...
    try {
      ...
      flashToast("تعذر الحفظ في GitHub، تأكد من صحة الـ Token");
    } finally {
      setGistBusy(false);
    }
  }
  ...
  async function restoreFromGist() {
    ...
    try {
      ...
      flashToast("تعذر الاسترجاع من GitHub، تحقق من الـ Token و Gist ID");
    } finally {
      setGistBusy(false);
    }
  }
  ...
  const totalArrears = data.members.reduce((s, m) => s + Math.min(m.arrears, 0), 0);
  ...
  function setMemberAmount(id, rawValue) {
    ...
    update((prev) => {
      ...
      return { ...prev, members: cleaned, balance: prev.balance + delta };
    });
  }
  ...
  function payArrears(id) {
    ...
    update((prev) => {
      const members = prev.members.map((m) => {
        if (m.id !== id || m.arrears >= 0) return m;
        return { ...m, arrears: Math.min(0, m.arrears + prev.monthlyDue) };
      });
      return { ...prev, members, balance: prev.balance + prev.monthlyDue };
    });
    flashToast("تم تسجيل تسديد 10 ريال من المتأخرات");
  }
  ...
  function submitWithdraw() {
    ...
    update((prev) => {
      if (isEdit) {
        const old = prev.withdrawals.find((w) => w.id === editWithdrawId);
        const oldAmount = old ? Number(old.amount) : 0;
        return {
          ...prev,
          balance: prev.balance + (oldAmount - amount),
  ...
  function removeWithdrawal(id) {
    if (!requireAdmin()) return;
    update((prev) => {
      const w = prev.withdrawals.find((x) => x.id === id);
      if (!w) return prev;
      return {
        ...prev,
        balance: prev.balance + Number(w.amount),
  ...
  function buildMessage() {
    ...
    lines.push(`رصيد الحالي *${fmt(data.balance)}*ريال`);
    lines.push("");
  ...
  function printAnnual() {
    const rowsHtml = data.members
      .map((m) => {
        ...
        return `<tr><td class="name">${escapeHtml(m.name)}</td>${cells}<td class="total">${fmt(yearTotal)}</td></tr>`;
      })
      .join("");
    const totalsRow = `<tr><td class="name total">الإجمالي الشهري</td>${monthTotals
      .map((t) => `<td class="total">${t > 0 ? fmt(t) : "—"}</td>`)
      .join("")}<td class="total grand">${fmt(grandTotal)}</td></tr>`;
    const headerCells = MONTHS_SHORT.map((m) => `<th>${m}</th>`).join("");
...
<p style="margin-top:16px;">الرصيد الحالي: ${fmt(data.balance)} ر.ع</p>
</body></html>`;
    ...
    const rows = data.withdrawals
      .map((w) => {
        ...
        return `<tr>
          ...
          <td class="total">${fmt(Number(w.amount))}</td>
        ...
        <div style={styles.sealWrap}>
          <div style={styles.seal}>
            <div style={styles.sealInner}>
              ...
              <span style={styles.sealAmount}>{fmt(data.balance)}</span>
          ...
          {editingHeader ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <div style={{ flex: 1 }}>
                  <label style={styles.fieldLabel}>الرصيد</label>
                  <input style={styles.input} type="number" value={data.balance} onChange={(e) => update((p) => ({ ...p, balance: Number(e.target.value) || 0 }))} />
                </div>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {m.arrears < 0 && (
                      <div style={styles.arrearsRow}>
                        <span style={styles.arrearsTag}>{fmt(m.arrears)} متأخرات</span>
                        <button style={styles.payArrearsBtn} onClick={() => payArrears(m.id)}>تسديد ١٠</button>
                      </div>
                    )}
                  </div>
      ...
      {tab === "stats" && (
        <>
          <div style={styles.summaryRow} className="no-print">
            ...
            <div style={styles.summaryChip}>
              <span style={styles.summaryNum}>{fmt(data.balance)}</span>
...
const styles = {
  ...
  arrearsRow: { display: "flex", alignItems: "center", gap: 8, marginTop: 4 },
